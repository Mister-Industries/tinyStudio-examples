 ...
 * Studio: https://app.tinystudio.cc/Mister-Industries/tinySpeak/Software/Arduino/Examples/tinyAIVoice
 *
...
/*
 * Project: tinySpeak - tinyAI_Voice
 ...
 * 1. Press [r] in Serial Monitor -> Records audio to SD Card (/rec.wav).
 * 2. Press [r] again -> Stops recording and kicks off pipeline:
 *    a. Audio sent to OpenAI Whisper (Speech-to-Text).
 *    b. Transcript sent to ChatGPT (GPT-4o-mini).
 *    c. Reply streamed to SD as MP3 via OpenAI TTS, then played from SD.
 *
 * Requirements:
...
int16_t wav_buffer[I2S_READ_LEN / 2];
...
void startRecording() {
    ...
    isRecording = true;
    Serial.println("Recording... Press [r] to stop.");
}
...
void captureAudio() {
    ...
    if (i2s_channel_read(rx_handle, raw_samples, sizeof(raw_samples), &bytes_read, 1000) == ESP_OK) {
        ...
        wavFile.write((uint8_t*)wav_buffer, stereo_frames * 2);
    }
}
...
String transcribeAudio() {
    ...
    client.println("Authorization: Bearer " + cfg_api_key);
